...
async function displayProfile() {
  try {
    ...
    profileContainer.innerHTML = `
      <div style="display: flex; flex-direction: column; align-items: center; gap: 1rem;">
        ...
        <div style="text-align: center;">
          <div id="profile-name" class="profile-name" style="font-size: 2rem; font-weight: 600; color: #f7fafc; margin-bottom: 0.5rem;">
            ${user?.name || 'User'}
          </div>
    ...
    // 저장: user object 및 id token claims
    window.__auth_user = user || {};
    window.__id_token_claims = idTokenClaims || {};

    const openMetadata = () => {
      const u = window.__auth_user || {};
      const claims = window.__id_token_claims || {};
      const userObjEl = document.getElementById('user-object');
      const claimsEl = document.getElementById('idtoken-claims');
      const backdrop = document.getElementById('metadata-backdrop');
      const panel = document.getElementById('metadata-panel');

      if (userObjEl) userObjEl.textContent = JSON.stringify(u, null, 2);
    ...
    if (pic && !pic.__metadata_attached) {
      pic.addEventListener('click', openMetadata);
      pic.__metadata_attached = true;
    }

    const closeBtn = document.getElementById('close-metadata');
    if (closeBtn && !closeBtn.__metadata_attached) {
      closeBtn.addEventListener('click', closeMetadata);
      closeBtn.__metadata_attached = true;
    }

    const backdrop = document.getElementById('metadata-backdrop');
    if (backdrop && !backdrop.__metadata_attached) {
      backdrop.addEventListener('click', closeMetadata);
      backdrop.__metadata_attached = true;
    }
  } catch (err) {
